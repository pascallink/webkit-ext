...
| Der Bestaetigungs-Pfad des Inline-Fallbacks in `src/otrsflow.js:setReference` ist ungetestet (Stand Sub-Task 3). | Die Fixture legt kein `form` um das Referenzfeld, daher laeuft `test/modules/otrs/browser/otrsflow.test.js` dort immer in den `sendKey('Enter')`-Rueckfall - getestet ist der Rueckfall, nicht der Primaerpfad. Synthetische `KeyboardEvent`s loesen keine Default-Action aus, Enter allein committet in echtem Jira also nichts; greift der Submit-Knopf nicht, meldet `run()` trotzdem `ok: true`. Zu beseitigen mit einem `form` samt `button[type="submit"]` in `mock-jira-otrs.html` und einem Testfall, der den Klick statt des Enters nachweist - spaetestens vor dem Release, zusammen mit dem manuellen Test gegen die echte Instanz. Gleiche Klasse: `pingJiraTrace()` laeuft in den Tests nur ueber den leeren catch-Zweig, es gibt keine Fixture mit `window.JIRA`. |
